...
-- Function computing first-order autoregreassive model and the Markov chain correlation for 9 lag
...
main = do
    ...
    pid1 <- forkProcess $ do
        ...
        let writepath = "./OutMP/" ++ mission ++ "/Lag9/ImageSplitting" 
            ...
            readpathO = readpath ++ "/Iref" ++ target ++"O.dat" 
        ...
        ref15 <- openFile readpathO ReadMode; contentsRef15 <- hGetContents ref15
...
-- Third process
    ...
    --
    -- ghc -O2 ImageSplitting_Multiprocessing_lag9.hs
    -- time ./ImageSplitting_Multiprocessing_lag9 +RTS -s 
