...
--         c = (zipxSY . mapV (mapSY (zipWithMat(\ x y -> mcSystem dimx dimy (signal [y]) (signal [x]) ) st )) . unzipxSY) sv
--         m1 = fromSignal c !! 0; cm1 = m1 `atV` 0
...
main = do
    ...
    let writepath = "/home/marcello-costa/workspace/ChangeDetection/Out/K1/Lag3/ParDai/CD0.txt" -- AF1
    ...
    tid1 <- forkIO $ do
        ...
        let readpatha = readpath ++ "/Iref" ++ target ++ "B.dat"
    ...
    tid2 <- forkIO $ do
        ...
        let readpathb = readpath ++ "/Iref" ++ target ++ "I.dat"
    ...
    tid3 <- forkIO $ do
        ...
        let readpathc = readpath ++ "/Iref" ++ target ++ "M.dat"
    ...
    --let out = vector [intest,(signal [m1]),intest,(signal [m2]), intest,(signal [m3])]
    ...
    --  ghc -O2 MTD3A.hs -threaded -rtsopts -eventlog
    -- ./MTD3A +RTS -N3 -ls; threadscope MTD3A.eventlog