...
typedef uint16_t pixel_t; //Define a 16-bit grayscale pixe;
...
int read_bmp(char *filename, unsigned char **header, pixel_t **data) { 
    ...
    data_tmp = malloc(size * sizeof(pixel_t));
    //fread (data_tmp, sizeof(struct pixel), size, file); // read the data
    fread(data_tmp, sizeof(pixel_t), size, file); // Read 16-bit grayscale data
    fclose (file);
...
void write_bmp(char *filename, unsigned char *header, pixel_t *data) {
    ...
    fwrite(data, sizeof(pixel_t), size, file);
    fclose(file);
}
...
int main(int argc, char *argv[]){
    ...
    *(stream_to_mem_dma+3) = 0;
    ...
    // wait for the user to examine the original image
